...
take_instant_full() {
    grim - | tee "$NAME" | wl-copy --type image/png && notify-send -t 1000 "Screenshot copied & saved to $screenshot_folder/$NAME"
...
take_instant_area() {
    ...
    grim -g "$region" - | tee "$NAME" | wl-copy --type image/png && notify-send -t 1000 "Screenshot copied & saved to $screenshot_folder/$NAME"
    [[ -f "$HOME/$NAME" && -d "$screenshot_folder" && -w "$screenshot_folder" ]] && mv "$HOME/$NAME" "$screenshot_folder/"
}
